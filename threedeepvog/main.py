...
def main(args):
    ...
    ques = {
        **make_queues(['model_inference', 'post_processing', 'ellipse_fitting']),
        'gaze_tracking': queue.Queue(maxsize=args['batch_size'] * 4) if args['gaze_tracking_flag'] else None,
        'torsion_tracking': queue.Queue(maxsize=args['batch_size'] * 4) if args['torsion_tracking_flag'] else None,
        'params_rendering': queue.Queue(maxsize=args['batch_size'] * 4) if args.get('fit_video_flag', False) else None,

        'ellipse_out': queue.Queue(maxsize=args['batch_size'] * 4),
        'gaze_out': queue.Queue(maxsize=args['batch_size'] * 4) if args.get('gaze_tracking_flag', False) else None,
        'torsion_out': queue.Queue(maxsize=args['batch_size'] * 4) if args['torsion_tracking_flag'] else None,
        
        'frame_out': queue.Queue(maxsize=args['batch_size'] * 4),  # raw frames for overlay video (batched)
        'segment_out': queue.Queue(maxsize=args['batch_size'] * 4) if args.get('seg_video_flag', False) else None,
        'seg_overlay_out': queue.Queue(maxsize=args['batch_size'] * 4) if args.get('seg_video_flag', False) else None,
        'fitted_frame_out': queue.Queue(maxsize=args['batch_size'] * 4) if args.get('fit_video_flag', False) else None,
