...
def get_conf_args(active_args):
    ...
    if active_args['mode'] == 'predict':
        input_vid = active_args['pred_vid']
    else:
    ...
    if active_args['eyeball_path'] is None:
        default_eyeball_folder = os.path.join(log_dir, "fit")
        save_path = os.path.join(default_eyeball_folder, f"model_params.json")
        active_args['eyeball_path'] = save_path
    else:
        if not os.path.exists(active_args['eyeball_path']):
            print(f"Error: Eyeball parameter file does not exist :(")
            sys.exit(1)  # Exit the script with error code 1

    vid_info = get_video_info_torch(input_vid)
...
def make_args():
    ...
    if is_ipython:
        return {
            ...
            'torsion_radial_pxl': 60,
            'torsion_n_subimgs': 100,
            'torsion_max_deg': 15.0,
            'torsion_subimg_angular_deg': 90.0,
            'torsion_subimg_h_pxl': 15,
            'torsion_coverage_rate_threshold': 0.7,
            'torsion_force_update_template_interval_sec': 5.0,
            'torsion_TM_algorithm': "stochastic",
            ...
            'fit_iris_ellipse': True,
            'max_eyeball_param_opt_iters': 10000,
            'batch_size': 32,

            # --- Thresholds ---
            'th_under_exposure': 0.1,
            'th_over_exposure': 0.9,
            'threshold_pupil': 0.5,
            'threshold_iris': 0.5,
            'threshold_glints': 0.5,
            'threshold_sclera': 0.5,
            'threshold_confidence_pupil': 0.985,
            'threshold_confidence_iris': 0.985,
            'blink_threshold': 0.735,

            # --- Camera Parameters ---
            'focal_length': 16.0,
            'focal_length_pxl': None,
            'sensor_size': [4.8, 3.6]
        }
        ...
        parser.add_argument('--model_frozen', action='store_true', default=True,
                            help="Freeze the eyeball model parameters (only for PL model)")
        ...
        parser.add_argument('--viz_torsion', action='store_true',
                            help="Visualize torsion overlays")
        ...
        parser.add_argument('--sensor_size', type=float, nargs=2, default=[4.8, 3.6],
                            help="Senso}r size (width height) in mm")
