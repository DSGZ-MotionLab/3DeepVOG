...
class GazeTracker(threading.Thread):
    def __init__(self, threads, args, daemon=False, use_queue=True):
        ...
        if self.args['write_fit_video']:
            ...
            self.large_meshes = gen_sphere_mesh(r_e, n_grids, insert_theta=limbus_theta_eye)
            self.small_meshes = gen_sphere_mesh(r_c, n_grids, insert_theta=limbus_theta_cornea)
    ...
    def gaze_tracker(self, frame_batch):
                ...
                if (els['confidence'][ix] == 0) or (np.isnan(els['center_x'][ix])) or\
                    ...
                    if self.args['write_fit_video']:
                        ...
                        result = fit_legrand_model(self.eyeball_params, df_gaze_out.iloc[0], processed_torsion,
                                                self.large_meshes, self.small_meshes, self.eyeball_params['num_grids'], use_mask=True)
                        fitted_frame = self.render_model_fitting(result, fitted_frame.copy())
        ...
        if self.use_queue:
            ...
            if self.args['do_torsion_tracking']:
            ...
            #         'frames': frames_np
            #     }
            #     self.threads['ques']['fitvideo_writer'].put(fitvideo_batch)
        else:
            return gaze_batch  
